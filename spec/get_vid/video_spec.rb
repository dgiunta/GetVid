...
module GetVid
  describe Video do
    before(:each) do
      ...
      @filename       = "mostafahendry-the_longest_yard_2006_here_comes_the_boom"
      ...
      @audio_output_file = File.join(fixture_path_for("TestOutput/Audio", true), @filename[0..26]) + ".aif"
    ...
    context "when first created" do
      ...
      it "should have a contributor" do
        @gv.send(:contributor).should == "mostafahendry"
      end
      ...
      it "should have an audio filename no longer than 31 characters" do
        ...
        @gv.send(:audio_filename).should == "mostafahendry-the_longest_y.aif"
      end
      ...
      it "should have an audio filepath" do
        @gv.send(:audio_filepath).should == File.expand_path(File.join(fixture_path_for("TestOutput/Audio"), @filename[0..26]) + ".aif")
      end
    end
  
    context "when downloading the video" do
      it "should send a get request to the url" do
        @gv.should_receive(:open).with(@url).and_return(@original_src)
        @gv.download
      end
    
      it "should send a get request to the keepvid.com site" do
        @gv.should_receive(:open).with(@keepvid_url).and_return(@keepvid_src)
        @gv.download
      end
    
      it "should download the file at the download_link location" do
        @gv.should_receive(:system).with("wget #{@download_link} -O #{@video_output_file}").and_return(true)
        @gv.download
      end
    end
  ...
  describe Video, "#from_links" do
    ...
    context "with an array of links" do
      ...
      it "should create an array of GetVid objects" do
        ...
        @gvs.first.should be_kind_of(Video)
      end
    end
